...
export default function ApiModal(props) {
  ...
  return (
    ...
    <div className='border bg-white flex flex-col p-8 rounded-md shadow-lg'>
      ...
      <div>
      <input value={props.apiKey} className='disabled italic no-underline text-slate-400 border rounded-md w-30rem mb-5 mt-3 px-3' type='text' placeholder='My Test Key'/>
      <button onClick={() => navigator.clipboard.writeText(props.apiKey)} className='border border-white rounded-md bg-emerald-600 hover:bg-emerald-700 text-white py-1 px-2'><FontAwesomeIcon icon={faCopy} /></button>
      </div>
      <div>
